...
struct SearchServie: APIService {
    ...
    static func searchInit2(aim: String?, area: String?, position: String?,department: String?, keyword: String?,completion: @escaping ([Search]) -> Void){
        let URL = url("/search")
        print(URL)
        let parameter = [
            "aim" : aim ?? "",
        ...
        Alamofire.request(URL, method: .get, parameters: parameter).responseData() { res in
            switch res.result {
            case .success:
                if let value = res.result.value {
                    let decoder = JSONDecoder()
