...
class LoginViewController: UIViewController, UITextFieldDelegate{
    ...
    @IBOutlet weak var loginCheckImg: UIImageView! // 자동로그인 기능
    var autoLogin = false
    ...
    func textFieldDidEndEditing(_ textField: UITextField, reason: UITextFieldDidEndEditingReason) {// 실행 취소 시
        if textField == idTf {
            if idTf.text == ""{
        ...
        if (idTf.text != "") && (pwTf.text != ""){
            loginBtn.setImage(#imageLiteral(resourceName: "btnLoginActive"), for: .normal)
        }
    }
    // 자동 로그인 버튼
    ...
    func logInit(){
        ...
        LoginService.logInit(email: idTf.text!,  password: pwTf.text!) {  (arr)  in
            if arr[0] == "login success"{
                if self.autoLogin == true {// 성공한 경우에 데이터 저장하기
            ...
            }else if arr[0] == "wrong password"{// alert
